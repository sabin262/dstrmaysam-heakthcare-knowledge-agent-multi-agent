...
SUPERVISOR_AGENT_TOOL_TO_AGENT = {
    "deterministic_lookup_agent": "DeterministicLookupAgent",
    "rag_agent": "RAGAgent",
    "policy_agent": "PolicyAgent",
    "catalog_agent": "CatalogAgent",
    "catalogue_agent": "CatalogAgent",
    "safety_agent": "SafetyAgent",
}
AGENT_TO_DEFAULT_INTENT = {
    "DeterministicLookupAgent": "postgres_deterministic_lookup",
    "RAGAgent": "document_search",
    "PolicyAgent": "policy_search",
    "CatalogAgent": "catalogue_search",
...
def _node_for_agent(agent_name: str) -> str:
    if agent_name == "DeterministicLookupAgent":
        return "deterministic_lookup"
    if agent_name == "PolicyAgent":
        return "policy"
...
def _specialist_task_from_route(
    route: dict[str, str],
    *,
    parent_query: str,
    task_id: str | None = None,
) -> SpecialistTask:
    tool_name = _canonical_tool_name(str(route.get("tool") or "rag_search"))
    query = str(route.get("query") or parent_query)
    agent = _agent_name_for_tool(tool_name)
    return SpecialistTask(
        ...
        constraints={
            ...
            "reason": str(route.get("reason") or "supervisor_rule_plan"),
        },
        required_output="specialist_report",
        parent_query=parent_query,
    )


def _specialist_task_to_dict(task: SpecialistTask) -> dict[str, Any]:
    return {
        ...
        "parent_query": task.parent_query,
    }


def _specialist_task_from_dict(data: dict[str, Any], *, parent_query: str) -> SpecialistTask:
    return SpecialistTask(
        task_id=str(data.get("task_id") or uuid.uuid4().hex),
        agent=str(data.get("agent") or "RAGAgent"),
        query=str(data.get("query") or parent_query),
        intent=str(data.get("intent") or ""),
        constraints=dict(data.get("constraints") or {}),
        required_output=str(data.get("required_output") or "specialist_report"),
        parent_query=str(data.get("parent_query") or parent_query),
    )


def _specialist_report_to_dict(report: SpecialistReport) -> dict[str, Any]:
    return {
        "task_id": report.task_id,
        "agent": report.agent,
        "status": report.status,
        "confidence": report.confidence,
        "answer_fragment": report.answer_fragment,
        "evidence": list(report.evidence),
        "tools_called": list(report.tools_called),
        "tool_execution_records": list(report.tool_execution_records),
        "recommended_next_agents": list(report.recommended_next_agents),
        "warnings": list(report.warnings),
        "latency_ms": report.latency_ms,
        "tool_context": report.tool_context,
        "catalog_guidance": list(report.catalog_guidance),
        "effective_query": report.effective_query,
    }


def _specialist_report_from_dict(data: dict[str, Any]) -> SpecialistReport:
    return SpecialistReport(
        ...
        tools_called=[str(tool) for tool in list(data.get("tools_called") or [])],
        tool_execution_records=list(data.get("tool_execution_records") or []),
        recommended_next_agents=[str(agent) for agent in list(data.get("recommended_next_agents") or [])],
        warnings=[str(warning) for warning in list(data.get("warnings") or [])],
        latency_ms=int(data.get("latency_ms") or 0),
        tool_context=str(data.get("tool_context") or ""),
        catalog_guidance=list(data.get("catalog_guidance") or []),
        effective_query=str(data.get("effective_query") or ""),
    )


def _task_key(task: SpecialistTask) -> tuple[str, str, str]:
    return (task.agent, _canonical_tool_name(str(task.constraints.get("planned_tool") or task.intent)), " ".join(task.query.lower().split()))


def _has_completed_agent(state: dict[str, Any], agent_name: str) -> bool:
    for report in list(state.get("specialist_reports") or []):
        if isinstance(report, dict) and report.get("agent") == agent_name:
            return True
    return False
...
class SpecialistTask:
    ...
    required_output: str = "specialist_report"
    parent_query: str = ""
...
class MultiAgentGraphContext:
    ...
    max_steps: int
    tool_names: set[str]
    deterministic_guard_queries: list[str]
    planned_guard_routes: list[dict[str, str]]


class SupervisorAgent:
    agent_name = "SupervisorAgent"

    def __init__(self, context: MultiAgentGraphContext):
    ...
    def run(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        if state.get("direct_answer"):
            return self.append_synthesis_decision(state, "llm_supervisor_direct_answer")
        if int(state.get("step_count") or 0) >= self.context.max_steps:
            return self.append_synthesis_decision(state, "max_agent_steps_reached")

        remaining_tasks = list(state.get("remaining_tasks") or [])
        if remaining_tasks:
            task_data = dict(remaining_tasks.pop(0))
            state["remaining_tasks"] = remaining_tasks
            return self.route_to_task(
                state,
        ...
        if state.get("tool_outputs") or state.get("specialist_reports"):
            recommended_task = self.next_recommended_task(state)
            if recommended_task is not None:
                return self.route_to_task(
                    state,
                    recommended_task,
                    reason="supervisor_report_arbitration",
                )
            if (
                _has_generic_info_intent(original_query)
                and self.deterministic_no_result_seen(state)
            ...
            if self.deterministic_guard_needed(state):
                return self.route_next_deterministic_guard(state, "supervisor_deterministic_guard_after_tool")
            if self.planned_guard_missing_routes(state, "supervisor_rule_guard_after_tool"):
                return self.route_next_planned_guard(state, "supervisor_rule_guard_after_tool")
            if (
                self.retrieval_no_result_seen(state, "policy_search")
                and "rag_search" not in list(state.get("tools_used") or [])
                and "document_search" not in list(state.get("tools_used") or [])
            ):
                return self.route_to_tool(
            ...
            if self.safety_review_needed(state):
                return self.route_to_task(
                    state,
                    SpecialistTask(
                        ...
                        constraints={"planned_tool": "safety_guard", "reason": "supervisor_safety_review"},
                        parent_query=original_query,
                    ),
                    reason="supervisor_safety_review",
                )
        ...
        response = self.context.owner._invoke_model(
            self.context.bound_llm,
            [_make_system_message(self.context.system_prompt), _make_human_message(routing_prompt)],
            self.context.config,
        )
        ...
        if tool_calls:
            ...
            for tool_call in tool_calls:
                ...
                call_query = _tool_query(_tool_call_args(tool_call), original_query)
                selected_agent = SUPERVISOR_AGENT_TOOL_TO_AGENT.get(call_name)
                if selected_agent:
                    tasks.append(
                        SpecialistTask(
                            task_id=uuid.uuid4().hex,
                            ...
                            query=call_query,
                            intent=AGENT_TO_DEFAULT_INTENT.get(selected_agent, "document_search"),
                            constraints={"reason": "llm_supervisor_agent_call"},
                            parent_query=original_query,
                        )
                    )
                    continue
                tool_name = _canonical_tool_name(call_name)
                routes.append(
                    {
                        "tool": tool_name,
                        "query": call_query,
                        "reason": "llm_supervisor_tool_call_compat",
                    }
                )
            ...
            if tasks:
                ...
                routes = [
                    ...
                    for route in routes
                    if (
                        _task_key(_specialist_task_from_route(route, parent_query=original_query)) not in existing_task_keys
                        and (
                            _agent_name_for_tool(str(route.get("tool") or "")),
                            _canonical_tool_name(str(route.get("tool") or "")),
                        )
                        not in existing_agent_intents
                    )
                ]
            tasks.extend(
                _specialist_task_from_route(route, parent_query=original_query)
                for route in routes
            )
            if not tasks:
                return self.append_synthesis_decision({**state, "performance": performance}, "llm_supervisor_empty_agent_selection")
            first_task = tasks.pop(0)
            state = {
                **state,
                "remaining_tasks": [_specialist_task_to_dict(task) for task in tasks],
                "performance": performance,
            }
            return self.route_to_task(
                state,
                first_task,
                reason=str(first_task.constraints.get("reason") or "llm_supervisor_agent_call"),
            )

        _add_timing(performance, "llm_direct_answer_ms", llm_ms)
        if self.deterministic_guard_needed(state):
            return self.route_next_deterministic_guard(
                {**state, "performance": performance},
                "supervisor_deterministic_guard_direct_answer",
            )
        if self.planned_guard_missing_routes(state, "supervisor_rule_guard_direct_answer"):
            return self.route_next_planned_guard(
                {**state, "performance": performance},
                "supervisor_rule_guard_direct_answer",
            )
        return self.append_synthesis_decision(
            {
                **state,
    ...
    def append_synthesis_decision(self, state: dict[str, Any], reason: str) -> dict[str, Any]:
        agent_flow = list(state.get("agent_flow") or [])
        if not (
            agent_flow
            and isinstance(agent_flow[-1], dict)
            and agent_flow[-1].get("agent") == self.agent_name
            ...
            agent_flow.append(
                {
                    ...
                    "kind": "supervisor",
                    "decision": "synthesize",
                    "selected_agent": SynthesisAgent.agent_name,
                    "reason": reason,
                }
            )
    ...
    def route_to_tool(
        ...
        *,
        ...
        tool_name = _canonical_tool_name(tool_name)
        task = _specialist_task_from_route(
            {"tool": tool_name, "query": query, "reason": reason},
            parent_query=self.context.original_query,
        )
    ...
    def route_to_task(self, state: dict[str, Any], task: SpecialistTask, *, reason: str) -> dict[str, Any]:
        ...
        return {
            **state,
            "pending_task": _specialist_task_to_dict(task),
            "pending_tool": "",
            "pending_query": task.query,
    ...
    def deterministic_guard_needed(self, state: dict[str, Any]) -> bool:
        ...
        pending_tool = str((pending_task.get("constraints") or {}).get("planned_tool") or pending_task.get("intent") or "")
        if _canonical_tool_name(pending_tool) in DETERMINISTIC_ROUTE_TOOLS:
            return False
        for task in list(state.get("remaining_tasks") or []):
            task = dict(task)
            task_tool = str((task.get("constraints") or {}).get("planned_tool") or task.get("intent") or "")
            if _canonical_tool_name(task_tool) in DETERMINISTIC_ROUTE_TOOLS:
                return False
        return True

    def safety_review_needed(self, state: dict[str, Any]) -> bool:
        if _has_completed_agent(state, "SafetyAgent"):
            return False
        if _has_safety_intent(self.context.original_query):
            return True
        for report_data in list(state.get("specialist_reports") or []):
            if not isinstance(report_data, dict):
                continue
            warnings = " ".join(str(warning) for warning in list(report_data.get("warnings") or [])).lower()
            if any(marker in warnings for marker in ("urgent", "unsafe", "phi", "escalation", "clinical safety")):
                return True
        return False

    def next_recommended_task(self, state: dict[str, Any]) -> SpecialistTask | None:
        completed = {
        ...
        pending_task = dict(state.get("pending_task") or {})
        pending_agent = str(pending_task.get("agent") or "")
        if pending_agent:
            queued.add(pending_agent)

        best_report: SpecialistReport | None = None
        for report_data in list(state.get("specialist_reports") or []):
            if not isinstance(report_data, dict):
                continue
            report = _specialist_report_from_dict(report_data)
            if report.status not in {"no_evidence", "partial", "not_my_domain"}:
                continue
            if not report.recommended_next_agents:
                continue
            if best_report is None or _task_status_priority(report.status) < _task_status_priority(best_report.status):
                best_report = report
        if best_report is None:
            return None

        for agent_name in best_report.recommended_next_agents:
            if agent_name in completed or agent_name in queued:
                continue
            if agent_name not in {"DeterministicLookupAgent", "RAGAgent", "PolicyAgent", "CatalogAgent", "SafetyAgent"}:
                continue
            return SpecialistTask(
                task_id=uuid.uuid4().hex,
                agent=agent_name,
                query=self.context.original_query,
                intent=f"arbitration_followup_from_{best_report.agent}",
                constraints={"reason": "specialist_recommended_next_agent"},
                parent_query=self.context.original_query,
            )
        return None

    def first_deterministic_guard_query(self) -> str:
        return (
            self.context.deterministic_guard_queries[0]
            if self.context.deterministic_guard_queries
            else self.context.original_query
        )

    def deterministic_guard_routes(self) -> list[dict[str, str]]:
        return [
            {
            ...
            for query in self.context.deterministic_guard_queries
        ]

    def planned_requires_only_deterministic(self) -> bool:
    ...
    def deterministic_specialist_query(self, query: str) -> str:
        if (
            ...
            return self.first_deterministic_guard_query()
        return query

    def apply_deterministic_guard_to_routes(self, routes: list[dict[str, str]]) -> list[dict[str, str]]:
        if not self.context.deterministic_guard_queries:
            return routes
        ...
        for route in routes:
            if route.get("tool") == "postgres_deterministic_lookup":
                normalized_routes.append(
                    {**route, "query": self.deterministic_specialist_query(str(route.get("query") or ""))}
                )
            else:
        ...
        if any(
            ...
            and any(self.route_queries_match(route, {"query": guard_query}) for guard_query in self.context.deterministic_guard_queries)
            for route in normalized_routes
        ):
            return normalized_routes
        guard_routes = self.deterministic_guard_routes()
        if self.planned_requires_only_deterministic():
            return guard_routes
        return [*normalized_routes, *guard_routes]

    def route_matches_planned(self, existing: dict[str, str], planned: dict[str, str]) -> bool:
        ...
        if existing_tool != planned_tool and not (
            ...
            return False
        return self.route_queries_match(existing, planned)

    def route_queries_match(self, existing: dict[str, str], planned: dict[str, str]) -> bool:
    ...
    def apply_planned_guard_to_routes(self, routes: list[dict[str, str]]) -> list[dict[str, str]]:
        normalized_routes = self.apply_deterministic_guard_to_routes(routes)
        ...
        for planned_route in self.context.planned_guard_routes:
            if planned_route.get("tool") == "policy_search":
                for index, route in enumerate(guarded_routes):
                    if route.get("tool") == "rag_search" and self.route_queries_match(route, planned_route):
                        guarded_routes[index] = {
                            **route,
                            "tool": "policy_search",
                            "query": planned_route.get("query") or route.get("query") or self.context.original_query,
                            "reason": "supervisor_rule_guard",
                        }
                        break
            if any(self.route_matches_planned(route, planned_route) for route in guarded_routes):
                continue
            reason = (
    ...
    def planned_guard_missing_routes(self, state: dict[str, Any], reason: str) -> list[dict[str, str]]:
                ...
                covered_routes.append(
                    {
                        ...
                        "query": str(step.get("query") or self.context.original_query),
                    }
                )
        pending_tool = str(state.get("pending_tool") or "")
        if pending_tool:
            covered_routes.append({"tool": pending_tool, "query": str(state.get("pending_query") or self.context.original_query)})
        pending_task = dict(state.get("pending_task") or {})
        if pending_task:
            pending_constraints = pending_task.get("constraints") if isinstance(pending_task.get("constraints"), dict) else {}
            covered_routes.append(
                {
                    "tool": str(pending_constraints.get("planned_tool") or pending_task.get("intent") or ""),
                    "query": str(pending_task.get("query") or self.context.original_query),
                }
            )
        ...
        for task in list(state.get("remaining_tasks") or []):
            if isinstance(task, dict):
                ...
                covered_routes.append(
                    {
                        "tool": str(constraints.get("planned_tool") or task.get("intent") or ""),
    ...
    def route_next_deterministic_guard(self, state: dict[str, Any], reason: str) -> dict[str, Any]:
        guard_routes = [
            {**route, "reason": reason}
            for route in self.deterministic_guard_routes()
        ]
    ...
    def route_next_planned_guard(self, state: dict[str, Any], reason: str) -> dict[str, Any]:
        guard_routes = self.planned_guard_missing_routes(state, reason)
        first_route = guard_routes.pop(0)
        state = {**state, "remaining_routes": guard_routes}
        return self.route_to_tool(
            state,
    ...
    def deterministic_no_result_seen(self, state: dict[str, Any]) -> bool:
...
class SpecialistGraphAgent:
    agent_name = "SpecialistAgent"
    node_name = "specialist"
    default_tool = "rag_search"
    allowed_tools: tuple[str, ...] = ("rag_search",)
    prompt: str = "Handle the assigned specialist task and return validated evidence."

    def __init__(self, context: MultiAgentGraphContext, supervisor: SupervisorAgent):
        self.context = context
        self.supervisor = supervisor

    def prepare_query(self, tool_name: str, query: str) -> str:
        return query
    ...
    def choose_tools(self, task: SpecialistTask, state: dict[str, Any]) -> list[str]:
        ...
        if planned_tool in self.allowed_tools and planned_tool in self.context.tool_names:
            return [planned_tool]
        return [tool for tool in self.allowed_tools if tool in self.context.tool_names][:1]

    def validate_report(
        self,
        *,
        task: SpecialistTask,
        tool_results: list[SpecialistAgentResult],
        started_at: float,
    ) -> SpecialistReport:
        tools_called = [tool for result in tool_results for tool in result.tools_used]
        sources = [source for result in tool_results for source in result.sources]
        catalog_guidance = [
            guidance
            for result in tool_results
            for guidance in result.catalog_guidance
        ]
        tool_context = "\n\n".join(result.tool_context for result in tool_results if result.tool_context)
        answer_fragment = "\n\n".join(result.answer_fragment for result in tool_results if result.answer_fragment)
        errors = [error for result in tool_results for error in result.errors]
        statuses = [result.status for result in tool_results] or ["failed"]
        if errors and not any(status == "ok" for status in statuses):
            status = "failed"
        elif any(status == "ok" for status in statuses):
            status = "partial" if any(status in {"no_evidence", "failed"} for status in statuses) else "answered"
        elif any(status == "no_evidence" for status in statuses):
            status = "no_evidence"
        else:
            status = "failed"
        confidence = {
        ...
        if status == "no_evidence":
            recommended = self.recommend_next_agents(task, tool_results)
        return SpecialistReport(
            task_id=task.task_id,
            agent=self.agent_name,
            status=status,
            ...
            recommended_next_agents=recommended,
            warnings=warnings,
            ...
            effective_query=str(
                next(
                    (
                        result.performance.get("effective_query")
                        for result in tool_results
                        if result.performance.get("effective_query")
                    ),
                    task.query,
                )
            ),
    ...
    def recommend_next_agents(self, task: SpecialistTask, tool_results: list[SpecialistAgentResult]) -> list[str]:
        return []

    def run(self, state: dict[str, Any]) -> dict[str, Any]:
        state = dict(state)
        started = time.perf_counter()
        task = _specialist_task_from_dict(
            dict(state.get("pending_task") or {}),
            parent_query=self.context.original_query,
        )
        if not task.agent or task.agent == "RAGAgent":
            task.agent = self.agent_name
        tools_to_call = self.choose_tools(task, state)
        tool_results: list[SpecialistAgentResult] = []
        agent_flow = list(state.get("agent_flow") or [])
        for tool_name in tools_to_call:
            tool_name = _canonical_tool_name(tool_name)
            query = self.prepare_query(tool_name, task.query)
            agent_flow.append(
                {
                    "agent": self.agent_name,
                    "kind": "specialist_tool_decision",
                    "task_id": task.task_id,
                    "tool": tool_name,
                    "query": query,
                    "reason": "autonomous_tool_selection",
                    "allowed_tools": list(self.allowed_tools),
                }
            )
            if tool_name not in self.context.tool_names:
                ...
                specialist_result = self.context.owner._run_specialist_agent(
                    tool_name,
                    query,
            ...
            agent_flow.append(specialist_flow)

        report = self.validate_report(task=task, tool_results=tool_results, started_at=started)
        agent_flow.append(
            {
                "agent": self.agent_name,
                ...
                "recommended_next_agents": report.recommended_next_agents,
                "warnings": report.warnings,
                "report_latency_ms": report.latency_ms,
            }
        )
        performance = dict(state.get("performance") or {})
        _add_tool_timing(performance, report.catalog_guidance)
        tools_used = list(state.get("tools_used") or [])
        tools_used.extend(report.tools_called)
        return {
            **state,
            "pending_tool": "",
            "pending_query": "",
            ...
            "sources": list(state.get("sources") or []) + list(report.evidence),
            "tool_outputs": list(state.get("tool_outputs") or []) + ([report.tool_context] if report.tool_context else []),
            "specialist_reports": list(state.get("specialist_reports") or []) + [_specialist_report_to_dict(report)],
            "catalog_guidance": list(state.get("catalog_guidance") or [])
            + list(report.catalog_guidance),
            "agent_flow": agent_flow,
            "performance": performance,
...
class DeterministicLookupAgent(SpecialistGraphAgent):
    ...
    def prepare_query(self, tool_name: str, query: str) -> str:
        if tool_name == "postgres_deterministic_lookup":
            return self.supervisor.deterministic_specialist_query(query)
        return query
    ...
    def validate_report(
        ...
        *,
        ...
        if not any(_deterministic_tool_output_has_results(result.answer_fragment) for result in tool_results):
            ...
            if _has_generic_info_intent(task.query):
                report.recommended_next_agents = ["RAGAgent"]
        return report


class RAGAgent(SpecialistGraphAgent):
    agent_name = "RAGAgent"
    node_name = "rag"
    default_tool = "rag_search"
    allowed_tools = ("document_search", "rag_search")
    prompt = "Choose document retrieval tools for general knowledge questions and validate answerable chunks."

    def choose_tools(self, task: SpecialistTask, state: dict[str, Any]) -> list[str]:
        if "document_search" in self.context.tool_names:
            return ["document_search"]
        return super().choose_tools(task, state)

    def recommend_next_agents(self, task: SpecialistTask, tool_results: list[SpecialistAgentResult]) -> list[str]:
        if _has_policy_intent(task.query):
            return ["PolicyAgent"]
        return []


class PolicyAgent(SpecialistGraphAgent):
...
class CatalogAgent(SpecialistGraphAgent):
    agent_name = "CatalogAgent"
    node_name = "catalog"
    default_tool = "catalogue_search"
    allowed_tools = ("catalogue_search", "document_catalog")
    prompt = "Choose catalog tools for document inventory and metadata questions only."

    def choose_tools(self, task: SpecialistTask, state: dict[str, Any]) -> list[str]:
        if "catalogue_search" in self.context.tool_names:
            return ["catalogue_search"]
        if "document_catalog" in self.context.tool_names:
    ...
    def validate_report(
        ...
        *,
        ...
        report = super().validate_report(task=task, tool_results=tool_results, started_at=started_at)
        if report.status == "answered" and not _has_catalog_intent(task.query):
            report.recommended_next_agents = ["PolicyAgent" if _has_policy_intent(task.query) else "RAGAgent"]
        return report


class SafetyAgent(SpecialistGraphAgent):
    agent_name = "SafetyAgent"
    node_name = "safety"
    default_tool = "safety_guard"
    allowed_tools = ("safety_guard",)
    prompt = "Review urgent, risky, PHI, escalation, and clinical safety-sensitive requests."
    ...
    def validate_report(
        ...
        *,
        ...
        if report.status == "no_evidence":
            ...
            report.warnings.append("Safety review returned limited evidence; synthesize conservatively.")
        return report
...
class SynthesisAgent:
    ...
    def run(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        deterministic_queries = [
            ...
            for step in list(state.get("agent_flow") or [])
            if isinstance(step, dict) and step.get("agent") == DeterministicLookupAgent.agent_name
        ]
        non_deterministic_queries = [
            ...
            and step.get("agent") != DeterministicLookupAgent.agent_name
        ]
        ...
        if direct_answer and not tool_context:
            ...
            answer = "\n\n".join([*deterministic_sections, *deterministic_no_result_sections]) or owner._offline_answer(
                query=original_query,
                tool_context=tool_context,
            )
            ...
            if not answer:
                answer = owner._offline_answer(query=original_query, tool_context=tool_context)
        ...
        performance["specialist_reports"] = list(state.get("specialist_reports") or [])
        ...
        result = GraphAgentResult(
            ...
            tools_used=tools_used,
            tool_context=owner._bounded_context(tool_context or "No graph tools were executed."),
...
class KnowledgeAgent:
    ...
    def _supervisor_agent_callables(self) -> list[Any]:
        definitions = [
            (
                "deterministic_lookup_agent",
                "Route exact Postgres operational facts, rota, patients, appointments, wards, contacts, equipment, formulary, counts, lists, and CRM table lookups.",
            ),
            (
                "policy_agent",
                "Route policies, SOPs, pathways, guidelines, compliance, privacy, confidentiality, governance, research data, retention, and procedures.",
            ),
            (
                "rag_agent",
                "Route general document-content questions that need indexed document chunks but are not specifically policy inventory.",
            ),
            (
                "catalog_agent",
                "Route document inventory, document list, category, type, metadata, and availability questions.",
            ),
            (
                "safety_agent",
                "Route urgent, risky, escalation, PHI, unsafe, or clinical safety-sensitive requests for review.",
            ),
        ]
        callables: list[Any] = []
        for name, description in definitions:
            def selector(query: str, _name: str = name) -> str:
                """Select a specialist agent for the supervisor graph."""
                return query

            selector.__name__ = name
            selector.__doc__ = description
            callables.append(selector)
        return callables
    ...
    def _call_langgraph_agent(
        ...
        *,
        ...
        bound_llm = llm.bind_tools(self._supervisor_agent_callables()) if hasattr(llm, "bind_tools") else llm
        ...
        multi_agent_context = MultiAgentGraphContext(
            owner=self,
            llm=llm,
            bound_llm=bound_llm,
            system_prompt=system_prompt,
            ...
            deterministic_guard_queries=deterministic_guard_queries,
            planned_guard_routes=planned_guard_routes,
        )
        supervisor_agent = SupervisorAgent(multi_agent_context)
        specialist_agents: dict[str, SpecialistGraphAgent] = {
            DeterministicLookupAgent.node_name: DeterministicLookupAgent(multi_agent_context, supervisor_agent),
            RAGAgent.node_name: RAGAgent(multi_agent_context, supervisor_agent),
            PolicyAgent.node_name: PolicyAgent(multi_agent_context, supervisor_agent),
            CatalogAgent.node_name: CatalogAgent(multi_agent_context, supervisor_agent),
            SafetyAgent.node_name: SafetyAgent(multi_agent_context, supervisor_agent),
        }
        synthesis_agent = SynthesisAgent(multi_agent_context)

        def run_without_langgraph() -> GraphAgentResult:
            state = supervisor_agent.initial_state()
            while True:
                state = supervisor_agent.run(state)
                node_name = next_node(state)
                if node_name == "synthesis":
                    state = synthesis_agent.run(state)
                    return state["result"]
                state = specialist_agents.get(node_name, specialist_agents[RAGAgent.node_name]).run(state)
        ...
        graph_builder.add_node("supervisor", supervisor_agent.run)
        graph_builder.add_node("deterministic_lookup", specialist_agents[DeterministicLookupAgent.node_name].run)
        graph_builder.add_node("rag", specialist_agents[RAGAgent.node_name].run)
        graph_builder.add_node("policy", specialist_agents[PolicyAgent.node_name].run)
        graph_builder.add_node("catalog", specialist_agents[CatalogAgent.node_name].run)
        graph_builder.add_node("safety", specialist_agents[SafetyAgent.node_name].run)
        graph_builder.add_node("synthesis", synthesis_agent.run)
        ...
        final_state = graph.invoke(supervisor_agent.initial_state())
